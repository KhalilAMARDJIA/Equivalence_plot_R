TOST_binom_plot <- function(x, n, Ref, Loweq, Higheq, conf.level) {
    ...
    if (CI.Low <= Loweq) { dd$Result <- "inconclusive" }
        ...
        if (CI.Low > Ref) { dd$Result <- "Superior" }
        else {
            if (CI.Low > Loweq & CI.High < Higheq) { dd$Result <- "Equivalent" }
    ...
    if (dd$Result == "Superior") { dd$Power <- Psup }
        ...
        if (dd$Result == "Not_inferior") { dd$Power <- PL }
        else {
            if (dd$Result == "Equivalent") { dd$Power <- P }
            else { dd$Power <- NA }
            }
    }
    ###############OUTPUTS################################################################################
